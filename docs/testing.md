...
| CLI render smoke: `status` never-run / running / stale, `--json`, `activity`, `config`, `account` | passing (2026-07-14, macOS) — was a **manual** smoke naming no test; the `status` half became automated with **W15-U1** (goldens for 9 states + the JSON shape, rows U1.1–U1.4), so only `activity`/`config`/`account` remain manual here |
...
| R13 | `QuarantineLocal` refuses a source that drifted since the scan — an edit landing mid-cycle wins the cycle instead of being removed ("edit beats delete", §4.2); since W13 the same pin guards the move to the bin, individually and inside a collapsed subtree (T13.6) | passing (2026-07-18; destination updated 2026-07-26) — `TestR13QuarantineLocalRefusesDriftedSource` + `TestR13QuarantineLocalProceedsWhenPinMatches` (executor), `TestR13MidCycleEditWinsOverQuarantine` (engine, resurrects to Drive next cycle) |
...
| R20 | Remote deletion of a folder whose local copy holds only ignored/temp leftovers (`.DS_Store`) removes it cleanly — leftovers travel with the dir; an unexpected survivor still refuses (was: `directory not empty` every cycle, forever) | passing (2026-07-19; destination updated 2026-07-26, W13) — `TestR20QuarantinedDirSweepsIgnoredLeftovers` (executor: ignored + temp both ride into the bin inside the folder), `TestR20DirWithoutTrashFallsBackToQuarantineSweep` (executor: the pre-W13 sweep-to-quarantine road, still exact), `TestR20QuarantinedDirStillRefusesUnexpectedSurvivor` (executor: real file refuses, untouched), `TestR20RemoteDeletedDirWithDSStoreUnwedges` (engine: one bin entry holding child + leftover, second cycle idle) |
| R21 | Two same-day rescues of one rel_path keep both copies (destination uniquifies; `moveFile` refuses an existing destination) (was: the second silently overwrote the first) | passing (2026-07-19; W13 2026-07-26 — the bin uniquifies the same way, T13.2) — `TestR21QuarantineNeverOverwritesRescueCopy` (executor, now pinned to the no-bin fallback road where `uniqueRescueDest` lives), `TestR21GuardedMoveFileRefusesOccupiedDestination` (belt-and-braces beneath the uniquifier), `TestR21SameDayRequarantineKeepsBothCopies` (engine: delete → restore → delete, both copies in the bin) |
...
| W3-adv-3 | A dead FSEvents stream after a volume remount is detected and recovered: `refresh` re-stats the root and reports it unwatchable when the device id changes (remount) or the root vanishes (unmount race), so the failure latch degrades to polling — honest `status` — and the recovery path recreates the stream on the current volume (parity with fsnotify's per-cycle re-walk); was: `needsRebuild=false` left a per-volume stream dead for the daemon's life while `status` still said "watching" | passing (2026-07-24, macOS) — `TestFSEventsRefreshDetectsRemount` (via the `fseventsRootDev` seam: unchanged dev → 0; changed dev → 1; unstattable root → 1; back to baseline → 0); red-first with the no-op `refresh`. Watch suite green under `-race` (cgo on) and `CGO_ENABLED=0` |
...
| DOC1 | The CLI command/flag surface is pinned to a golden manifest, so adding/renaming/removing a command or flag fails the build until the manifest — and the matching MANUAL §3 / spec §15 rows — are updated in the same commit (doc-code drift guard; born from the 2026-07-24 doc audit that found MANUAL §6 claimed skips are "shown in `status`" when the daemon never surfaces them) | passing (2026-07-24; extended 2026-07-25) — `TestCLISurfaceMatchesManifest` (`cmd/synckeeper/surface_test.go`: walks the real `newRootCmd()` tree — now **materializing cobra's built-ins** via `InitDefaultHelpCmd`/`InitDefaultCompletionCmd`/`InitDefaultVersionFlag` — and asserts the full 15-command set incl. `help`/`completion`, each command's non-inherited flags, and the global `-v/--verbose` + `--version`. The 2026-07-25 extension closed a blind spot: the pre-Execute enumeration had missed the built-ins, which is why `help`/`completion` slipped out of MANUAL §3) |
| W5.5-S3 | The daemon tightens its launchd log to `0600` (owner-only) at `watch` startup — the log records synced file *names* and launchd creates it `0644`; a `0644` file is tightened to `0600`, and an empty or missing path (foreground run, non-file-log platform) is a safe no-op | passing (2026-07-24) — `TestRestrictToOwner` (0644→0600, empty+missing no-op), `TestLogPathDarwin` (`internal/service`; `service.RestrictLogToOwner` wired into `cmd/synckeeper/watch.go`) |
| W11 | `synckeeper info` renders a complete static snapshot (version, all config-file paths, sync dir, Drive folder + id, machine name + id, OAuth client, token status, effective config, local state) in human + `--json`; works before init (hint, no empty `(id )`) and after; read-only/offline | passing (2026-07-25) — `TestPrintInfoHuman` (all fields present), `TestPrintInfoHumanNotInitialized` (hint + no empty id suffix + token-absent), `TestPrintInfoJSON` (valid JSON, nested `paths`/`state`); `info` in the DOC1 surface manifest (`cmd/synckeeper`) |
| W5.5-S4 | Pre-publication secret-scan gate (`make audit`): the tracked tree has no committed secret (Google OAuth/API key, AWS/Slack token, PEM private key — only the audit package's own pattern-holding source is allowlisted; `credentials.go` is **not** exempt since no embedded secret exists — spec §9 reversal 2026-07-25), no tracked `token.json`/`credentials.json`/`client_secret_*.json`/`*.db`, and `.gitignore` still covers them; detector proven to fire so the gate isn't vacuously green | passing (2026-07-24) — `TestSecretScanGate` (real tree clean), `TestScanBytesDetects` (planted fake secret caught, clean text + binary skipped), `TestSensitiveTrackedClassifier`, `TestRequiredGitignorePresent` (`internal/audit`; runs inside `go test ./...`) |
...
### W15 — `status` becomes a live TUI dashboard (2026-07-27 →)

| ID | Case | Status |
|---|---|---|
| U1.1 | **The plain view is byte-identical to the pre-dashboard renderer**, for all 9 daemon states (never-run, stopped, stale, running/watching, polling-only, paused, guard-blocked, last-error, no-system-bin) — the characterization net that lets the interactive view be built around it | passing (2026-07-27) — `TestPrintHumanMatchesPreW15Goldens` (`internal/status`), against goldens **captured from the pre-W15 `printStatusHuman`** by a throwaway generator that swapped `os.Stdout` for a pipe (generator deleted in the same commit). Fixture offsets are all ≥ 61 s and off a minute boundary because `Dur` truncates with `int()`. `TestPrintHumanCoversEveryDaemonState` fails if a state has no golden. **Also verified end to end on live daemon state:** the pre-refactor and post-refactor binaries print identical bytes for `status` **and** `status --json` against the running Mac daemon |
| U1.2 | `status --json` keeps every key it had — it is a scripting contract the dashboard must not touch | passing (2026-07-27) — `TestJSONViewShapeIsStable` (12 top-level keys, 11 `daemon.*` keys incl. `last_cycle`, activity row count) |
| U1.3 | Daemon classification: a live control socket beats a stale heartbeat; fresh heartbeat → running; stale → stale; clean shutdown → stopped; no row or no start time → never-run; a zero staleness window never ages a daemon out | passing (2026-07-27) — `TestDaemonStateClassification` (6 cases + the zero-window case) with an injected clock; the policy (`3 × watch.HeartbeatInterval`) stays in `cmd`, the logic in `internal/status` |
| U2.1 | Every dashboard frame is golden-pinned, with colour off so a diff is a layout change and never a palette change: overview / activity / info / help, plus overview at 60 columns, guard-blocked + no-bin, stale + last-error, never-run, paused | passing (2026-07-27) — `TestViewGoldens` (9 frames, `internal/tui`, `-update` rewrites them), `TestNoEscapeCodesWithColorOff`. Fixtures pin a fixed **UTC** instant (`testRef`) — the header shows a wall clock and `Snapshot.Now` is the only clock a frame reads, so goldens are byte-stable in any timezone |
| U2.2 | No frame ever overflows the terminal width, in any view, at 40/60/80/100/140 columns — found and fixed three real bugs (the header did not truncate, activity rows had a path-width floor that broke the contract, and an info note was appended after the value was truncated) | passing (2026-07-27) — `TestFrameNeverOverflowsWidth`, `TestNarrowLayoutStacksPanels` (two columns at 100, stacked at 60, no line over 60). Columns are **dropped, not squeezed**: an activity row loses the direction label, then the kind; the header drops the clock, then trailing segments — whole segments, so a cut never lands inside an escape sequence |
| U2.3 | The attention panel is absent on a healthy machine and, when something is wrong, names the way out (not just the diagnosis): guard block + `--confirm-deletes`, no system bin, missing credentials, autostart not installed | passing (2026-07-27) — `TestAttentionOnlyWhenSomethingIsWrong`. A permanently visible "all good" box trains people to ignore the space where warnings appear |
| U2.4 | The next-poll figure is presented as an **estimate** (`≈`), never a deadline — it is recorded at cycle end while the poll ticker runs independently and a local change pre-empts it | passing (2026-07-27) — `TestNextPollIsMarkedAsAnEstimate`; the precise countdown waits for U5 |
| U2.5 | Keys: `1`/`2`/`3` select views, Tab cycles and wraps, `?` toggles help and selecting a view closes it, `r` refreshes now, `q` and `ctrl+c` quit; a resize is adopted; a tick refreshes and reschedules; a model with no refresher neither starts nor reschedules a ticker | passing (2026-07-27) — `TestKeysNavigateAndQuit`, `TestResizeAndTickAreHandled` |
| U2.6 | **The real bubbletea program runs headless** — starts, calls `Refresh` for a full first frame, renders, switches view on a keystroke, and exits on `q` — and a cancelled context (Ctrl-C / SIGTERM) is a clean exit, not an error | passing (2026-07-27) — `TestRunRendersThenQuits`, `TestRunTreatsCancellationAsCleanExit`, through the `Input`/`Output` seams with an `os.Pipe` (bubbletea's cancellable reader needs a real fd). `-race` green |
| U2.7 | **The dashboard is read-only by construction:** no file in `internal/tui` may call `statedb.Open` (writable, migrates — spec §14/R5), any `os` path mutator, or `trash.MoveToTrash` | passing (2026-07-27) — `TestDashboardIsReadOnlyByConstruction` (AST walk, in the local-write gate's idiom), and `TestGuardCatchesAPlantedViolation` proves the guard can fail |
| U2.8 | Output routing: a terminal gets the dashboard, a pipe/redirect gets the one-shot report, `--plain` forces the report on a terminal, `--json` is unchanged; `--watch` is gone | passing (2026-07-27) — `TestCLISurfaceMatchesManifest` (the DOC1 guard **fired** on the flag change and was satisfied together with MANUAL §3 + §10 and spec §15), plus a live check on the Mac: piped output still matches the pre-W15 report, `status --watch` now errors `unknown flag` |
| U1.4 | Shared display formatters render against an injected instant, so no view reads the clock mid-frame: `Ago`/`Until` sentinels ("never", "unknown", "now"), `Dur` buckets (s/m/h+m/d+h), unknown direction → empty, `CycleSuffix` on empty and malformed JSON, `AutostartText` when the service is absent | passing (2026-07-27) — `TestFormatters` (`internal/status`) |
...
| T13.10 | macOS: `NSFileManager trashItemAtURL:` trashes a real item; a missing path errors; the trash is always available | **passing (2026-07-26, macOS/arm64, go 1.26.4, cgo on)** — `TestMoveToTrashDarwin` (opt-in, `SYNCKEEPER_TRASH_TEST=1`: real file into `~/.Trash`, source gone), `TestMoveToTrashDarwinMissing`, `TestAvailableDarwin`. First compilation of `trash_darwin.go`/`.m` anywhere — clean, no cgo/linker change needed. Full `go build ./... && go vet ./... && go test ./... && make audit` green on the Mac in the same run |
| T13.11 | A folder deleted in Drive leaves **one** bin entry holding the whole subtree, the sync dir clean, every covered row retired, and the next cycle idle | passing (2026-07-26) — `TestT13RemoteFolderTrashLandsInBinAsOneEntry` (engine, 9 files + an untouched sibling), `TestCollapsedDirTrashedAsOneEntry` (executor: rows + journal), `TestR20RemoteDeletedDirWithDSStoreUnwedges` (the leftover rides along inside the entry) |
| T13.12 | The collapse pass: a directory absorbs its subtree only when every action under it is delete-class; any upload/download/record/move beneath it cancels it; the highest qualifying directory wins; siblings collapse independently; a lone directory delete is left untouched | passing (2026-07-26) — `TestCollapseAbsorbsFullyDeletedSubtree`, `TestCollapseRefusedWhenSubtreeHasOtherWork`, `TestCollapseAllowsOtherDeleteClassActions`, `TestCollapseKeepsOnlyTheHighestDirectory`, `TestCollapseLeavesLoneDirectoryUntouched`, `TestCollapseHandlesSiblingsAndLooseFiles` (reconcile, pure) |
| T13.5 | The mass-delete guard still counts a collapsed subtree's **files** (A2's mirror image: counting containers wedged the daemon once, counting *nothing* would wave a whole tree through) — the guard runs before the collapse **and** reads `SubtreeFiles`, so the ordering cannot silently become load-bearing | passing (2026-07-26) — `TestT13CollapseKeepsTheMassDeleteGuardCounting` (engine: 12 of 13 files trashed in Drive → blocked, bin empty, tree intact), `TestT13GuardCountsACollapsedSubtreesFiles` (guards, pure), `TestMassDeleteGuardBlocksWholeTreeDeletion` (W12's rows, now collapse-aware) |
| T13.6 | Anything the plan never reasoned about refuses the collapse and falls back to per-item: a file created after the scan is left untouched (the covered files are still rescued, the op fails, rows stay for the replan), and a covered file edited after the scan keeps "edit beats delete" (§4.2, R13) | passing (2026-07-26) — `TestCollapsedDirRefusesUnexpectedSurvivor`, `TestCollapsedDirRefusesEditedFile`, `TestR20QuarantinedDirStillRefusesUnexpectedSurvivor` (executor). Red-first: with `verifySubtree` disabled all three fail |
| T13.4 | No usable bin (unavailable platform, or a bin that refuses the move) falls back to the dated quarantine, item by item, and nothing is hard-deleted; rescue copies still uniquify | passing (2026-07-26) — `TestCollapsedDirWithoutTrashFallsBackToQuarantine` (both modes), `TestR20DirWithoutTrashFallsBackToQuarantineSweep`, `TestR21QuarantineNeverOverwritesRescueCopy` (executor, fallback road) |
| T13.7 | A crash between the move to the bin and the DB commit converges on the next run — no resurrection, no lost content (invariant 6) | passing (2026-07-26) — `TestT13CrashBetweenTrashAndCommitConverges` (engine, `executor.CPTrashBeforeCommit`) |
| T13.16 | `activity` never claims a deletion the mass-delete guard stripped: a guard-blocked cycle records only the actions that ran (found in the field 2026-07-26 — a folder held by the guard was reported as trashed once every 45 s while it sat in the sync dir) | passing (2026-07-26) — `TestActivityNeverClaimsGuardBlockedDeletes` (watch). Red-first: before the fix it recorded both the collapsed `trash` and the `trash_remote` |
| T13.13 | `activity` reports one `trash` entry per subtree with its file count, and names the quarantine instead when there is no bin; `info` shows the bin destination | passing (2026-07-26) — `TestActivityReportsOneTrashEntryPerSubtree`, `TestActivityNamesTheQuarantineWhenThereIsNoBin` (watch), `TestPrintInfoHuman`/`TestPrintInfoJSON` (`system bin` line + `paths.system_bin`) |
| T13.14 | The local-write gate covers the bin: no file outside `localwrite.go` may call `MoveToTrash` (spec §7 — handing a path to the trash removes it as surely as `os.Rename`) | passing (2026-07-26) — `TestGateNoRawFSMutationOutsideLocalwrite` (extended) |
| T13.15 | No test may reach the developer's own bin: both suites pin their default trash to a temporary `trash.Fake` | passing (2026-07-26) — `TestMain` in `internal/engine` and `internal/executor`; per-machine `Fake` bins in the engine harness |
| T13.17 | **On-hardware acceptance, both platforms** (W13 + W14 closing criterion): a folder deleted in the Drive web UI is removed from every machine with no confirmation and arrives as **one** restorable entry in that machine's own bin | **passing (Linux 2026-07-26, macOS 2026-07-27)** — not an automated row: **Linux** (`pop-os`) observed by the agent in the journal, one cycle `actions=1 executed=1 failed=0` for a 1117-file folder, bin entry confirmed by Max; **macOS** confirmed by Max on the rebuilt binary ("tested the linux and mac machine … all works as expected"). Re-run per platform port (Windows owes it with W8's recycle-bin backend) |
...
| W12-mixed | The incident's race — folder trashed in Drive while `rm -rf` is part-way through the local tree — splits cleanly: `forget` for rows gone on both sides, `quarantine_local` for rows still on disk, **never an upload** | passing (2026-07-25) — `TestRemoteTrashDuringPartialLocalDeletePlansNoUpload` (40 of 60 removed locally). Structurally the field cycle (890 executed + 255 others), which makes the deviation precise: the field's 255 came out as **uploads**, so those files had **no baseline rows**, while these do |
