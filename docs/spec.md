...
1. **Reconcile:** every decision-table row covered by table-driven tests; move pairing **in both directions, files and directories** (R9); directory ordering; dependency ordering of §4.5 (regression tests R1, R2); move/backup destination-overwrite guard (R7, R8); rows resolved at post-move paths (R11); no rel_path overlap in the concurrent transfer stage (R12).
