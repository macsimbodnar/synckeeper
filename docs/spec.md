...
9. **Soak:** 2-hour random-edit soak on both sides converges with no divergence — part of the release ritual, re-run per platform port. *(W3.5, 2026-07-23: `TestSoak` now runs against the production watcher backend — FSEvents on darwin+cgo; 30 s/90 s smokes converge with clean `doctor`. The full 2 h gate on FSEvents is Max's release-ritual run. The original 2 h pass was 2026-07-09 on fsnotify.)*
