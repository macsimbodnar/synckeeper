...
**W1.8 — adversarial correctness fixes, round 2** ([plan.md](plan.md) W1.8), **in the listed order**. Items 1–7 are **done**. **Next is item 8 — [A9 + A8] two low-severity hardening items**: (a) a crashed directory `MoveLocal` — renamed on disk, DB not yet committed — plans `TrashRemote` for the dir when it is empty (`plan.go:374-377`: `hasCreateUnder` and `remoteAliveUnder` both false); converges next cycle, but a crash produced a remote delete, the shape invariant 6 rules out; test R16. (b) `remotedelta.Snapshot`'s BFS has no visited set (unlike `prune`'s directly above), so a parent cycle in the cache is an infinite loop in the daemon; test R17. Then item 9's remainder — one small named test (`TestSnapshotDuplicateNameCollision`, `internal/remotedelta`). Discipline is W1.7's: **red-first regression test, then the fix, then the testing.md row**, suite + `-race` green at every commit.
