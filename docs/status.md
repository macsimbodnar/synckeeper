...
**W1.9 — adversarial round-3 fixes** ([plan.md](plan.md) W1.9), **five items in listed order**: 1. cross-tree fold conflicts (C2, test R19 — targeted fold-match: fold-keyed sibling index fires the ordinary adopt/conflict; fold-shadowed baseline rows are skips, never remote-deletes); 2. the quarantine sweep (C3/R20 — ignored/temp leftovers travel to quarantine with the dir); 3. rescue-copy protection (C4/R21); 4. type-clash skips (C5/R22); 5. PKCE (C6/R23). Each fix's commit retires its MANUAL.md Known-bugs entry (items 1–4 are the four entries still listed). Discipline unchanged: **red-first regression test, then the fix, then the testing.md row**, suite + `-race` green at every commit. Also on the table from W1.8's close-out, unscheduled: the dir arm of the R7 occupied-destination rule (see "Last completed").
