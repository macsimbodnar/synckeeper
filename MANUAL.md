...
**The countdown is exact when the daemon can tell you.** A running daemon publishes its real poll deadline, whether a cycle is running (`syncing  now · 12s`), whether a local change is already in flight (`next sync  local change — syncing`), and which watch backend is live — shown in the header, e.g. `watching (FSEvents)`. If the daemon is older than the dashboard, or not running, the figure falls back to the database and is **marked `≈`**: that number is recorded when a cycle ends, while the poll timer runs independently and any local change pre-empts it, so the real next sync is usually *sooner*.
