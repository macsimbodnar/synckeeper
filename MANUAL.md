...
- **Being fixed (found 2026-07-30 on macOS): a file you just uploaded can be moved to your system bin, then downloaded again.** Seen with a 190 MB video: `activity` showed `upload` → `trash` → `download` for the same file. Cause: Synckeeper's cached picture of your Drive folder is refreshed from Google's change feed, and the file it has just uploaded is not in that feed for a moment — so a sync cycle landing in that gap reads "not on Drive" as "deleted on Drive" and rescues your local copy to the bin. It normally cannot happen (cycles are 45 s apart, the gap is under a second); it takes a second cycle firing immediately after an upload. **Nothing is lost:** the file stays on Drive, the local copy goes to your bin, and the next sync downloads it back. *Cost: the file transfers twice and a stale copy sits in your Trash — safe to empty. If you hit it: nothing to do, it self-corrects on the next sync.*
